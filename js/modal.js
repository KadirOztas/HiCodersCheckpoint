  ...
  function editItem(index, detailsArray, renderFunction, detailFields) {
    const itemToEdit = detailsArray[index];
    const fieldInputs = detailFields.map((field) => ({
      id: field.id,
      label: field.label,
      type: field.type,
      value: itemToEdit[field.id],
    }));

    promptWithModal("Edit Item", fieldInputs, (...values) => {
      fieldInputs.forEach((field, i) => {
        itemToEdit[field.id] = values[i];
      });
  ...
  export { addModal, promptWithModal, createCard, deleteItem, editItem, StudentCard };
